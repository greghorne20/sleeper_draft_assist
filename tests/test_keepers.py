...
def rule(board=None):
    return keepers.eligible_keepers(KEEPER_PICKS, KEEPER_ROSTERS, KEEPER_USERS,
                                    KEEPER_TRANSACTIONS, PLAYERS, board)
...
def test_failed_transactions_are_ignored():
    ...
    assert any(t["status"] == "failed" and "1001" in (t["drops"] or {})
               for t in KEEPER_TRANSACTIONS), "fixture: a failed drop of 1001"
...
def test_team_with_no_eligible_players_renders_explicitly():
    ...
    md = keepers.render_keepers_md(report, {"for_league_name": "L", "for_season": "2026",
                                            "target_season": "2025"})
...
def test_end_to_end_writes_both_files(tmp_path, monkeypatch, players_cache):
    class Client:
        ...
        def get_league(self, league_id):
            return {"league_id": league_id, "season": "2026" if league_id == "L2026" else "2025",
                    "name": "Test League", "previous_league_id": "L2025"}
    ...
    monkeypatch.setattr(keepers, "walk_back", lambda c, lid, back, season: (
        c.get_league("L2025"),
        [{"league_id": "L2026", "season": "2026", "name": "Test League"},
         {"league_id": "L2025", "season": "2025", "name": "Test League"}],
    ))
    out = tmp_path / "draft"
    sys.argv = ["sleeper-keepers", "--league-id", "L2026", "--back", "1",
                "--cache-dir", str(players_cache), "--out-dir", str(out),
                "--board", str(tmp_path / "no-board.json"), "--weeks", "10"]
